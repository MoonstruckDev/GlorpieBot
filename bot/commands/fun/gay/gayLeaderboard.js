...
async function ensureLeaderboardExists(client, guildId, channelId) {
  ...
  // Ensure object exists and has keys set
  data[guildId] ??= { channelId: null, messageId: null };

  // Always store channelId even if messageId is null
  data[guildId].channelId = channelId;

  const channel = await client.channels.fetch(channelId).catch(() => null);
  if (!channel) return;

  // If we already have a messageId, verify it exists
...
async function updateLeaderboard(client, guildId) {
  ...
  try {
    ...
    // Re-create the message if it was deleted
