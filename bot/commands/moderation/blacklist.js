...
function denyOwner(interaction) {
  return interaction.reply({
    content:
      "<:GlorpNerd:1269929450712203264> Only the bot owner can manage global blacklist.",
    flags: MessageFlags.Ephemeral,
  });
}
...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (sub === "list") {
      if (!hasModPerms(interaction)) return denyPerms(interaction);
    ...
    if (sub === "add") {
      ...
      if (globalRequested) {
        ...
        blacklistService.addWord(guildId, word, true);

        return interaction.reply({
          content: `🌐 Added \`${word}\` to **global blacklist**.`,
          flags: MessageFlags.Ephemeral,
        });
      }
      ...
      blacklistService.addWord(guildId, word, false);

      return interaction.reply({
        content: `✅ Added \`${word}\` to **guild blacklist**.`,
        flags: MessageFlags.Ephemeral,
      });
    }
    ...
    if (sub === "remove") {
      ...
      if (globalRequested) {
        ...
        const result = blacklistService.removeWord(guildId, word, true);

        return interaction.reply({
          content: result
            ? `🗑️ Removed \`${word}\` from **global blacklist**.`
      ...
      const result = blacklistService.removeWord(guildId, word, false);

      return interaction.reply({
        content: result
          ? `🗑️ Removed \`${word}\` from **guild blacklist**.`
          : "<:GlorpNerd:1269929450712203264> Word not found.",
        flags: MessageFlags.Ephemeral,
      });
    }
